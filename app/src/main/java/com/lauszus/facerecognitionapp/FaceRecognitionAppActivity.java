        ...
        if (!uniqueLabelsSet.isEmpty()) { // Make sure that there are any labels
            ...
            if(answer){
                builder.setNeutralButton("More info", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        //dialog.dismiss();
                        Intent moreInfoIntent = new Intent(getApplicationContext(), MoreInfoActivity.class); // Remove last image
                        moreInfoIntent.putExtra("com.lauszus.facerecognitionapp.SEARCH_TEXT", searchText);
            ...
            if (minDist != -1) {
                ...
                //    Log.i(TAG, "dist[" + minIndex + "]: " + minDist + ", face dist: " + faceDist + ", label: " + imagesLabels.get(minIndex));
                    ...
                    if (faceDist < faceThreshold && minDist < distanceThreshold){ // 1. Near face space and near a face class
                        showToast("Face detected: " + imagesLabels.get(minIndex) + ". Distance: " + minDistString+" for more info click on More Info button", Toast.LENGTH_LONG);
                        answer = true;
                        String[] tokens = imagesLabels.get(minIndex).split(" ");
                        String temp = "";

                        for(int i = 0; i < tokens.length; i++){
                            if(i < tokens.length - 1){
                                temp += (tokens[i].substring(0, 1).toUpperCase() + tokens[i].substring(1)) + " ";
                            }else{
                                temp += (tokens[i].substring(0, 1).toUpperCase() + tokens[i].substring(1));
                            }
                        }
